...
namespace HackathonReembolso.Framework.Helpers
{
    ...
    /// <summary>
    /// Classe desenvolvida para compatibilidade com o sistema legado
    /// </summary>
    public class HelperCrypt
    {
        ...
        public HelperCrypt(CryptProvider cryptProvider)
        {
            ...
            if (_algorithm.LegalKeySizes.Length > 0)
            {
                ...
                else if (keySize < maxSize)
                {
                    ...
                    if (keySize < validSize)
                    {
                        ...
                        _key = _key.PadRight(validSize / 8, Convert.ToChar("*"));
                    }
                }

            }

            var key = new PasswordDeriveBytes(_key, ASCIIEncoding.ASCII.GetBytes(salt));
            return key.GetBytes(_key.Length);
        }

        public string Encrypt(string texto, out long ticket)
        {
            ticket = DateTime.Now.Ticks;
            _key = ticket.ToString();
